...
parser = argparse.ArgumentParser(description = 'Programa de consulta del instituto de arte.')
parser.add_argument('-c','--config',type=str, default='a',help = 'Campos consultar, puede ser en forma de lista o archivo .yml')
parser.add_argument('-o', '--out', default='json',help = 'Tipo de archivo con la consulta, pdf o json')
parser.add_argument('-d', '--dry_run', type=bool, default=False,required= False, help= 'Indica si se va a mandar')
'''
...
def info_galeria ():
    ...
    os.system("curl https://api.artic.edu/api/v1/artworks | jq '.data | length' > longitud.txt")
    ...
    os.system("curl https://api.artic.edu/api/v1/artworks | jq '.data[0] | keys' | jq '.[]' > parametros_busqueda.txt")
...
def ejecuta_consulta(numero_obras : int, parametros_busqueda: list) -> dict :
    ...
    for i in parametros_busqueda:
        ...
        for ii in range(0, numero_obras, 1) :
            os.system(f"curl https://api.artic.edu/api/v1/artworks | jq '.data[{ii}].{i}' > consulta.txt")
            with open('consulta.txt', 'r') as f:
                resultado_consulta_raw = f.read().replace('\n','').replace('"','')
...
def envia_archivo (destino : str, nombre_archivo : str) :
    ...
    with open(nombre_archivo, 'r') as f:
        ...
        adjunto['Contenet-Disposition'] = f'adjunto; nombre_archivo = "{nombre_archivo}"'
...
def ordena_configuracion (dict_config: dict) -> any:
    ...
    return dict_config['fields'], dict_config['name'], dict_config['max_items'], dict_config['recipients']
...
if __name__ == '__main__':
    ...
    if type(configuracion) == dict:
        print(configuracion)
        try: 
            lista_busqueda_usuario, nombre, max, email_destino = ordena_configuracion (configuracion)
        except Exception:
            print('Algo a salido mal al acceder a los datos de configuracion')
        else: 

            parametros_disponibles, cantidad_obras =info_galeria()
            busqueda_final = depura_busqueda(lista_busqueda_usuario,parametros_disponibles)
